...
In this seminar, we will focus on the paper by [Burnside & Dollar (2000)](http://www.jstor.org/stable/117311?seq=1#page_scan_tab_contents) and try to replicate their result. As the quote above illustrates, their main argument is that the effect of aid on economic growth is contingent on how sound the policies for a given country is. The units of analysis is country periods. 
...
We look at the original model (shown above) and give a standard R formula to the `lm()` function with the varibles shown in the table.
...
```{r stargazer_comp, tidy=FALSE, results='asis'}
